...
GitLab Codex Reviewer는 소규모 팀이 GitLab Merge Request와 branch commit의 1차 코드 리뷰를 Codex에 맡길 수 있도록 만든 self-hosted 웹 앱입니다.

GitLab webhook으로 변경을 감지하고, Reviewer Bot 계정으로 diff와 repository context를 읽은 뒤, Codex 리뷰 결과를 GitLab 댓글로 남깁니다. 개발자는 UI에서 프로젝트, 리뷰 브랜치, 리뷰 전략, Codex 연결 상태, 실패한 리뷰 재시도를 관리할 수 있습니다.
...
2. admin이 Settings에서 Reviewer Bot Token과 Codex 계정을 연결합니다.
...
- [Codex 설정](docs/codex-setup.md): Codex 연결, `.data/codex`, headless/Docker 주의사항
