...
Project의 `MR target branches`가 비어 있으면 MR 리뷰를 실행하지 않습니다. 값이 있으면 target branch별로 조회하고 같은 MR은 `iid` 기준으로 중복 제거합니다.
...
- 비어 있는 `MR target branches`는 MR 리뷰 대상 branch를 추가하지 않습니다.
...
Codex 리뷰 전에 worker는 checkout된 workspace에서 read-only 보조 분석을 실행합니다.

- `rg-risk-scan`: hardcoded secret, broad catch, 보안 TODO 같은 위험 패턴 후보를 찾습니다.
- `gitleaks`: secret 후보를 redaction된 JSON 결과로 요약합니다.
- `golangci-lint`: `.golangci.*` 설정이 있는 Go repository에서만 실행합니다.
- `eslint`: ESLint 설정이 있는 JS/TS repository에서 실행합니다. workspace local eslint가 없으면 앱에 번들된 eslint를 fallback으로 사용합니다.

ToolRunner 결과는 Codex prompt에 “검증해야 할 후보”로만 전달됩니다. 도구가 없거나 대상 repository 의존성이 부족하면 리뷰 전체를 실패시키지 않고 skip/failed 이벤트만 timeline에 남깁니다.
...
- MR review 대상 target branch 설정. 비워두면 MR 리뷰는 비활성화됩니다.
