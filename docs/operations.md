...
worker는 Next web process와 분리된 별도 Node process에서 주기적으로 실행됩니다.
...
4. Reviewer Bot Token으로 opened non-draft MR과 branch commit 상태를 조회합니다.
...
10. finding이 없으면 `no_findings`로 기록하고 댓글은 남기지 않습니다.
11. finding이 있으면 Reviewer Bot Token 계정으로 MR 일반 note 또는 commit comment를 작성합니다.
...
Commit review는 `Commit review branches`가 설정된 subscription이 있는 shared project에서만 자동 실행됩니다.
...
5. finding이 없으면 `no_findings`로 기록하고 댓글은 남기지 않습니다.
6. finding이 있으면 GitLab commit comment를 작성합니다.
...
- `Scan now`로 현재 로그인한 사용자 project만 즉시 polling 실행
...
worker는 Reviewer Bot Token이 없으면 GitLab 조회와 댓글 작성을 수행하지 않습니다.
